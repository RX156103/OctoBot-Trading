...
class Book:
    ...
    def _handle_book_update(self, order):
        new_size = Decimal(order.get(ECOBIC.SIZE.value, INVALID_PARSED_VALUE))
        new_price = Decimal(order.get(ECOBIC.PRICE.value, INVALID_PARSED_VALUE))
        price = Decimal(order[ECOBIC.PRICE.value])
        ...
        if order[ECOBIC.SIDE.value] == TradeOrderSide.BUY.value:
            ...
            if bids is None or order_index == ORDER_ID_NOT_FOUND:
                return
            if new_size != INVALID_PARSED_VALUE:
                bids[order_index][ECOBIC.SIZE.value] = new_size
            if new_price != INVALID_PARSED_VALUE:
                bids[order_index][ECOBIC.PRICE.value] = new_price
            self._set_bids(price, bids)
            return
        ...
        if asks is None or order_index == ORDER_ID_NOT_FOUND:
            return
        if new_size != INVALID_PARSED_VALUE:
            asks[order_index][ECOBIC.SIZE.value] = new_size
        if new_price != INVALID_PARSED_VALUE:
            asks[order_index][ECOBIC.PRICE.value] = new_price
        self._set_asks(price, asks)
...
def _order_id_index(order_id, order_list):
    ...
    for i in range(order_list):
        if order_list[i][ECOBIC.ORDER_ID.value] == order_id:
            return i
